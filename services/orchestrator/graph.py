...
PROMPTS = {
    "static_analysis": (
        ...
        "issues, unused variables, and poor naming. Return only a JSON array. Each "
        "item must have keys: file, line, severity (info/warning/error), message."
    ),
    "security": (
        "You are a security scanner. Review this git diff for OWASP Top 10 "
        "vulnerabilities, hardcoded secrets, and SQL injection risks. Return only a "
        "JSON array. Each item must have keys: file, line, severity, message."
    ),
    "architecture": (
        "You are an architecture reviewer. Review this git diff for separation of "
        "concerns violations, missing error handling, and improper dependency usage. "
...
def _style_prompt(state: GraphState) -> str:
    ...
    return (
        ...
        "readability, and consistency issues. Common patterns this team has had "
        f"before: {patterns_str}. Return only a JSON array. Each item must have "
        "keys: file, line, severity, message."
    )
